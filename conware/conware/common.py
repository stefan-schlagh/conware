...
def build_hardware(avatar, args):
    """
    Initialize our hardware object with the given arguments
    :param avatar:
    :param args:
    :return:
    """
    ...
    if "ivt_unlock" in args.__dict__ and args.ivt_unlock:
        hardware.ivt_unlock = args.ivt_unlock
    return hardware


def build_qemu_emulator(avatar, args):
    """
    Initialize our QEMU object wwith the specific arguments
    :param avatar:
    :param args:
    :return:
    """
...
def set_memory_map(avatar, args, model=False, pretender_model=None):
    qemu = avatar.get_target('qemu')
    hardware = None
    try:
        hardware = avatar.get_target('hardware')
    except:
        l.error("No hardware present")
...
def auto_int(x):
    """
    Parse any integer form (e.g., 0x10, 16, 0b11)
    :param x:
    :return:
    """
    return int(x, 0)
